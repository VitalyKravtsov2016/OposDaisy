...
type
  ...
  TDatecsStatus = record
    ...
    CoverOpened: Boolean;
    RAMCorrupted: Boolean;
    ReturnRecOpened: Boolean;
    RAMCleared: Boolean;
...
begin
  ...
  Status.CoverOpened := TestBit(B[1], 5);
  Status.RAMCorrupted := TestBit(B[1], 4);
  Status.ReturnRecOpened := TestBit(B[1], 3);
  Status.RAMCleared := TestBit(B[1], 2);
  Status.InvalidCommandInMode := TestBit(B[1], 1);
  Status.AddOverflow := TestBit(B[1], 0);
